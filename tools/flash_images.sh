...
#
# Supported platforms:
# LS1021ATWR, LS1012ARDB, LS1012AFRWY, LS1028ARDB, LS1043ARDB, LS1046ARDB, LS1046AFRWY, LS1088ARDB_PB, LS2088ARDB, LX2160ARDB
#
...
if test -z "$img"; then
    ...
    echo To flash single image: setenv img to bl2, fip, mcfw, mcdpc, mcdpl, fman, qe, dp, pfe, phy, dtb, linux_itb
    ...
    echo fip_img, dtb_img, linux_itb_img, qe_img, dp_img, fman_img, phy_img, mcfw_img, mcdpl_img, mcdpc_img
    exit
fi
...
if test -z "$linux_itb_img"; then
    if test $board = ls1021atwr; then
	linux_itb_img=/lsdk_linux_arm32_LS_tiny.itb
    else
...
elif test $board = ls1012ardb -o $board = ls1028ardb -o $board = ls1046ardb -o $board = ls1046afrwy -o $board = ls1088ardb_pb  -o $board = lx2160ardb; then
    # QSPI flash on LS1012ARDB, LS1046ARDB,  LS1046AFRWY and LS1088ARDB_PB
    addr_bl2=0x0
    addr_fip=0x00100000
    addr_ddr=0x00800000
...
if test $board = ls1046ardb -o $board = ls1046afrwy -o $board = ls1088ardb_pb -o $board = lx2160ardb; then    # probe QSPI flash
    if test $bank = other; then
...
# DPAA2 MC firmware
if test $board = ls1088ardb_pb -o $board = ls2088ardb -o $board = lx2160ardb; then
    if load $bd_type $bd_part $img_load_addr $mcfw_img && $pt_off_cmd $addr_mcfw $efs && $erase_cmd $addr_mcfw +$filesize && $write_cmd $img_load_addr $addr_mcfw $filesize; then
	echo Success: flashed $mcfw_img to $addr_mcfw
    else
	echo Failed to flash $mcfw_img
...
elif test $board = ls1028ardb -o $board = ls1088ardb_pb -o $board = ls2088ardb -o $board = lx2160ardb; then
