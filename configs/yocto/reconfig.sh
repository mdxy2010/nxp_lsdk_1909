...
if [ $1 = fixcfg ]; then
    # reconfigure busybox in yocto-poky
    sed -i 's/# CONFIG_DEVMEM is not set/CONFIG_DEVMEM=y/' $FBDIR/packages/rfs/yocto-poky/meta/recipes-core/busybox/busybox/defconfig
    sed -i 's/# CONFIG_TC is not set/CONFIG_TC=y/' $FBDIR/packages/rfs/yocto-poky/meta/recipes-core/busybox/busybox/defconfig

    # add yocto custom packages with recipes
    sudo cp -rf $FBDIR/packages/rfs/misc/yocto/recipes-support/* $FBDIR/packages/rfs/yocto-poky/meta/recipes-support/

    # reconfigure machine serial console
    [ $DESTARCH = arm64 ] && cfgfile=qemuarm64.conf || cfgfile=qemuarm.conf
    sed -i -e 's/ttyAMA0/ttyS0/' -e 's/hvc0//' $FBDIR/packages/rfs/yocto-poky/meta/conf/machine/$cfgfile
fi
...
if [ $1 = fixsys ]; then
    ...
    sudo mkdir -p $RFSDIR/etc/rpm-postinsts && sudo cp -f $FBDIR/packages/rfs/misc/yocto/100-sysvinit-inittab $RFSDIR/etc/rpm-postinsts
    ...
    if ! grep -q ^S1 $RFSDIR/etc/inittab; then \
        sudo sed -i '/S0/a\S1:12345:respawn:/bin/start_getty 115200 ttyS1 vt102\nAMA0:12345:respawn:/bin/start_getty 115200 ttyAMA0 vt102' $RFSDIR/etc/inittab
    fi
